...
class ProductCartController extends Controller
{
    ...
    public function maxQuantity($id)
    {
        return Product::find($id)->quantity_available;
    }


    public function changeQuantity(Request $request)
    {
        $u = ProductCart::find($request->id)->update(['quantity'=>$request->quantity]);
        return response(
            ['id'=> $request->id,
            'quantity'=> $request->quantity,$u]
        );
    }
